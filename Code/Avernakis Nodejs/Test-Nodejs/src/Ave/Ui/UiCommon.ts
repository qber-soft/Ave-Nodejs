...
export class Hotkey {
	...
	Refine(): Hotkey {
		...
		if (InputModifier.Control & this.Modifier) this.Modifier |= InputModifier.Control;
		if (InputModifier.Shift & this.Modifier) this.Modifier |= InputModifier.Shift;
		if (InputModifier.Menu & this.Modifier) this.Modifier |= InputModifier.Menu;
		if (InputModifier.Sys & this.Modifier) this.Modifier |= InputModifier.Sys;
		return this;
	}
}
...
export class DpiSize {
	...
	GetScaledValue(nDpi: number) {
		return this.IsDpiScaled ? DpiSize.Scale(this.RawValue, nDpi) : this.RawValue;
	}
...
export class DpiMargin {
	...
	static FromPixel(l: number, t: number, r: number, b: number) {
		return new DpiMargin(DpiSize.FromPixel(l), DpiSize.FromPixel(t), DpiSize.FromPixel(r), DpiSize.FromPixel(b));
	}
	static FromPixelScaled(l: number, t: number, r: number, b: number) {
		return new DpiMargin(DpiSize.FromPixelScaled(l), DpiSize.FromPixelScaled(t), DpiSize.FromPixelScaled(r), DpiSize.FromPixelScaled(b));
	}
}
...
export class Rect {
	...
	Intersect(rc: Rect, result?: Rect) {
		if (rc.x < this.Right && rc.Right > this.x && rc.y < this.Bottom && rc.Bottom > this.y) {
			if (result) {
