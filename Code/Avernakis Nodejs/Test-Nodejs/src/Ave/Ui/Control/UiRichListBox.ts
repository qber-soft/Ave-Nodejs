...
export interface IRichListBox extends IControl {
	...
	ItemSetSelectRange(nStart: number, nEnd: number, bSelect: boolean): RichListBox; // in the range [nStart, nEnd], not [nStart, nEnd)
	ItemSort(nSubItem: number, bAsc: boolean, bCaseInsensitive: boolean): RichListBox;
	ItemFind(szString: string, nIndex: number, bExact: boolean, bCaseInsensitive: boolean): number;
	ItemSet(item: RichListBoxItem): boolean;
	ItemGet(nFlag: RichListBoxItemFlag, nIndex: number, nSubIndex: number): RichListBoxItem;
	ItemGetCount(): number;
	...
	ItemHitTest(pt: Vec2, bMustOnContent: boolean, bIncludePlaceholder: boolean): RichListBoxHitTestResult;
	...
	SetVirtual(fn: (sender: RichListBox, nIndex: number, nSubIndex: number) => RichListBoxItemVirtual): RichListBox;
	...
	OnDoubleClick(/**/ fn: (sender: RichListBox, nIndex: number) => void): RichListBox;
	OnRightClick(/**/ fn: (sender: RichListBox, nIndex: number) => void): RichListBox;
	OnDragBegin(/**/ fn: (sender: RichListBox) => void): RichListBox;
	OnGlyphClick(/**/ fn: (sender: RichListBox, nIndex: number) => void): RichListBox;
	OnItemCheck(/**/ fn: (sender: RichListBox, nIndex: number) => void): RichListBox;
}
