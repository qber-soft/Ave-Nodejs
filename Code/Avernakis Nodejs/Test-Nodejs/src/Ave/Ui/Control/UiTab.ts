...
export interface ITab extends IControl {
	...
	OnDoubleClick(fn: (sender: Tab, nId: number, nType: TabObjectType) => void): Tab;
	OnRightClickOnMiddleClick(fn: (sender: Tab, nId: number, nType: TabObjectType) => void): Tab;
	OnMiddleClick(fn: (sender: Tab, nId: number, nType: TabObjectType) => void): Tab;
	OnDrag(fn: (sender: Tab, nId: number, nType: TabObjectType, vDragStartPosition: Vec2) => void): Tab;
	OnButtonClick(fn: (sender: Tab, nId: number, nType: TabObjectType, nButtonIndex: number) => void): Tab;
}
...
export class Tab extends (AveLib.UiTab as ITab) {
	...
	OnDrag(fn: (sender: Tab, nId: number, nType: TabObjectType, vDragStartPosition: Vec2) => void): Tab {
		return super.OnDrag((sender: Tab, nId: number, nType: TabObjectType, vDragStartPosition: Vec2) => fn(sender, nId, nType, Vec2.FromNative(vDragStartPosition)));
	}
