...
export class NoteIndexUtil {
	...
	static CountWhiteKey(v: NoteRange) {
		return Math.max(0, Math.floor(v.y / 12) - Math.floor(v.x / 12) - 1) * 7 + 8 - NoteIndexUtil.__WhiteCountArray[v.x % 12] + NoteIndexUtil.__WhiteCountArray[v.y % 12];
	}
...
export interface IMidiKey extends IControl {
	...
	OnClick(fn: (sender: MidiKey, key: MidiKeyItem, bIsFirstPress: boolean, nModifier: InputModifier) => void): MidiKey;
	OnDrop(fn: (sender: MidiKey, key: MidiKeyItem, bIsLastRelease: boolean, nModifier: InputModifier) => void): MidiKey;
}
