...
export interface IMatrixEditorDoc {
	...
	SetVirtualUnit(fn: (sender: IMatrixEditorDoc, x: number, y: number) => MatrixUnit): IMatrixEditorDoc;
	SetVirtualColHeader(fn: (sender: IMatrixEditorDoc, n: number) => string): IMatrixEditorDoc;
	SetVirtualRowHeader(fn: (sender: IMatrixEditorDoc, n: number) => string): IMatrixEditorDoc;
	...
	ViewSetValueFormat(fn: (sender: IMatrixEditorDoc, x: number, y: number, v: number) => string);
...
export interface IMatrixEditor extends IControl {
	...
	OnUnitClick(fn: (sender: MatrixEditor, x: number, y: number, u: MatrixUnit) => void): MatrixEditor;
	OnUnitRightClick(fn: (sender: MatrixEditor, x: number, y: number, u: MatrixUnit) => void): MatrixEditor;
	OnUnitChange(fn: (sender: MatrixEditor, x: number, y: number, u: MatrixUnit) => void): MatrixEditor;
	OnUnitChangeBegin(fn: (sender: MatrixEditor, x: number, y: number, u: MatrixUnit) => void): MatrixEditor;
	OnUnitChangeEnd(fn: (sender: MatrixEditor, x: number, y: number, u: MatrixUnit, bCanceled: boolean) => boolean): MatrixEditor; // return false to reject the change, bCanceled is the recommanded behavior.
}
...
export class MatrixEditor extends (AveLib.UiMatrixEditor as IMatrixEditor) {
	...
	DocGet(): IMatrixEditorDoc {
		...
		doc.GetUnitBackColor = () => Vec4.FromNative(OriginalGetUnitBackColor());

		const OriginalGetUnitForeColor = doc.GetUnitForeColor.bind(doc);
		doc.GetUnitForeColor = () => Vec4.FromNative(OriginalGetUnitForeColor());

		const OriginalGetUnitFillColor = doc.GetUnitFillColor.bind(doc);
		doc.GetUnitFillColor = () => Vec4.FromNative(OriginalGetUnitFillColor());
