...
export interface IRibbonBackstage extends IControl {
	...
	OnSelect(fn: (sender: RibbonBackstage, nIndex: number) => void): RibbonBackstage;
	OnHighlight(fn: (sender: RibbonBackstage, nIndex: number) => void): RibbonBackstage;
}
...
export class RibbonBackstage extends (AveLib.UiRibbonBackstage as IRibbonBackstage) {
	...
	SetContent(nIndex: number, pControl: IControl) {
		if (nIndex < this.GetCount()) {
			while (nIndex < this.m_Content.length) this.m_Content.push(null as IControl);
