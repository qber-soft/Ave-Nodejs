...
export class Mat44 {
    ...
    get r(): Vec4[] {
        return [
            new Vec4(this._11, this._12, this._13, this._14),
            new Vec4(this._21, this._22, this._23, this._24),
            new Vec4(this._31, this._32, this._33, this._34),
            new Vec4(this._41, this._42, this._43, this._44),
        ];
    }
    ...
    Determinant() {
        return (
            this._11 *
                (this._22 * (this._33 * this._44 - this._43 * this._34) -
                    this._23 * (this._32 * this._44 - this._42 * this._34) +
                    this._24 * (this._32 * this._43 - this._42 * this._33)) -
            this._12 *
                (this._21 * (this._33 * this._44 - this._43 * this._34) -
                    this._23 * (this._31 * this._44 - this._41 * this._34) +
                    this._24 * (this._31 * this._43 - this._41 * this._33)) +
            this._13 *
                (this._21 * (this._32 * this._44 - this._42 * this._34) -
                    this._22 * (this._31 * this._44 - this._41 * this._34) +
                    this._24 * (this._31 * this._42 - this._41 * this._32)) -
            this._14 *
                (this._21 * (this._32 * this._43 - this._42 * this._33) -
                    this._22 * (this._31 * this._43 - this._41 * this._33) +
                    this._23 * (this._31 * this._42 - this._41 * this._32))
        );
    }
    ...
    Decompose(vTranslation: Vec3, vScaling: Vec3, q: Quat): boolean {
        ...
        if (vScaling.v[b] < decEpsilon) {
            const [aa, bb, cc] = this.__Decompose(
                Math.abs(t[a].x),
                Math.abs(t[a].y),
                Math.abs(t[a].z)
            );
    ...
    static RotateNormal(vNormal: Vec3, f: number) {
        ...
        const d = 1 - c;
        return new Mat44(
            d * vNormal.x * vNormal.x + c,
            s * vNormal.z + d * vNormal.x * vNormal.y,
            d * vNormal.z * vNormal.x - s * vNormal.y,
            0,
            d * vNormal.x * vNormal.y - s * vNormal.z,
            d * vNormal.y * vNormal.y + c,
            s * vNormal.x + d * vNormal.y * vNormal.z,
            0,
            s * vNormal.y + d * vNormal.z * vNormal.x,
            d * vNormal.y * vNormal.z - s * vNormal.x,
            d * vNormal.z * vNormal.z + c,
            0,
            0,
            0,
            0,
    ...
    static RotationQuaternion(q: Quat) {
        return new Mat44(
            1 - q.y * (q.y + q.y) - q.z * (q.z + q.z),
            q.x * (q.y + q.y) + q.w * (q.z + q.z),
            q.x * (q.z + q.z) - q.w * (q.y + q.y),
            0,
            q.x * (q.y + q.y) - q.w * (q.z + q.z),
            1 - q.x * (q.x + q.x) - q.z * (q.z + q.z),
            q.y * (q.z + q.z) + q.w * (q.x + q.x),
            0,
            q.x * (q.z + q.z) + q.w * (q.y + q.y),
            q.y * (q.z + q.z) - q.w * (q.x + q.x),
            1 - q.x * (q.x + q.x) - q.y * (q.y + q.y),
            0,
            0,
            0,
            0,
    ...
    static OrthographicOffCenterLH(
        fViewLeft: number,
        fViewRight: number,
        fViewBottom: number,
        fViewTop: number,
        ...
        return new Mat44(
            ...
            fRange,
            0,
            -(fViewLeft + fViewRight) * ReciprocalWidth,
            -(fViewTop + fViewBottom) * ReciprocalHeight,
            -fRange * fNearZ,
            1
        );
    }

    static OrthographicOffCenterRH(
        fViewLeft: number,
        fViewRight: number,
        fViewBottom: number,
        fViewTop: number,
