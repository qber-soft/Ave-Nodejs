...
export class Quat extends Vec4 {
	...
	Mul(r: Quat) {
		return new Quat(r.w * this.x + r.x * this.w + r.y * this.z - r.z * this.y, r.w * this.y - r.x * this.z + r.y * this.w + r.z * this.x, r.w * this.z + r.x * this.y - r.y * this.x + r.z * this.w, r.w * this.w - r.x * this.x - r.y * this.y - r.z * this.z);
	}
	...
	Exp() {
		...
		if (Math.abs(s) >= 0.0000001) {
			const coeff = s / theta;
			return new Quat(coeff * this.x, coeff * this.y, coeff * this.z, Math.cos(theta));
		} else {
	...
	Ln() {
		if (Math.abs(this.w) < 1) {
			...
			if (Math.abs(s) >= 0.0000001) {
				const coeff = angle / s;
				return new Quat(coeff * this.x, coeff * this.y, coeff * this.z, 0);
			}
		}
	...
	static RotateRollPitchYaw(pitch: number, yaw: number, roll: number) {
		...
		const sinR = Math.sin(0.5 * roll),
			cosR = Math.cos(roll);
		return new Quat(cosP * sinY * sinR + sinP * cosY * cosR, -sinP * cosY * sinR + cosP * sinY * cosR, -sinP * sinY * cosR + cosP * cosY * sinR, sinP * sinY * sinR + cosP * cosY * cosR);
	}

	static RotateAxis(axis: Vec3, angle: number) {
		const s = Math.sin(0.5 * angle);
		return new Quat(s * axis.x, s * axis.y, s * axis.z, Math.cos(0.5 * angle));
	}
	...
	static Slerp(q0: Quat, q1: Quat, s: number, bShortestPath: boolean = false) {
		let c = q0.Dot(q1);
		let t: Vec4;
	...
	static Squad(q0: Quat, q1: Quat, q2: Quat, q3: Quat, s: number, bShortestPath: boolean = false) {
...
export class DualQuat {
	...
	static FromQuaternionTranslation(q: Quat, v: Vec3) {
		return new DualQuat(q, new Quat(0.5 * (v.x * q.w + v.y * q.z - v.z * q.y), 0.5 * (-v.x * q.z + v.y * q.w + v.z * q.x), 0.5 * (v.x * q.y - v.y * q.x + v.z * q.w), -0.5 * (v.x * q.x + v.y * q.y + v.z * q.z)));
	}
}
