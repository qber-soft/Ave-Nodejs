type DerivedVecType<N> = N extends 2
    ? Vec2
    : N extends 3
    ? Vec3
    : N extends 4
    ? Vec4
    : Vec;
type FixedLengthArray<T, N extends number = number> = N extends 1
    ? [T]
    : N extends 2
    ? [T, T]
    : N extends 3
...
class Vec<
    Dimension extends number = number,
    DataType extends number[] = FixedLengthArray<number, Dimension>,
    VecType extends Vec = DerivedVecType<Dimension>
> {
    ...
    Add(value: Vec | number): VecType {
        return Reflect.construct(
            this.constructor,
            this.v.map((each, index) =>
                value instanceof Vec ? each + value.v[index] : each + value
            )
        );
    }
    ...
    Dot(value: Vec) {
        return this.v.reduce(
            (prev, curr, index) => prev + curr * value.v[index],
            0
        );
    }
    Length() {
    ...
    Normalize(): VecType {
        return Reflect.construct(this.constructor, this.v).Mul(
            1.0 / this.Length()
        );
    }
