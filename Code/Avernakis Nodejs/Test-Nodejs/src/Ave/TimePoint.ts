...
export class TimePoint {
    ...
    static get OAMinTick() {
        return (
            (TimePoint.DayPer100Year - TimePoint.DayPerYear) *
            TimePoint.TickPerDay
        );
    } // 1/1/0100
    ...
    static Day(nYear: number) {
        return TimePoint.IsLeapYear(nYear)
            ? TimePoint.Day366
            : TimePoint.Day365;
    }
    static DayTotal(nYear: number) {
        return TimePoint.IsLeapYear(nYear)
            ? TimePoint.DayTotal366
            : TimePoint.DayTotal365;
    }
    ...
    Set(
        ...
        if (
            ...
            nMonth <= 12 &&
            nDay >= 1 &&
            nDay <= TimePoint.DayInMonth1(nYear, nMonth)
        ) {
            const y = nYear - 1;
            this.Tick =
                (y * 365 +
                    ...
                    Math.floor(y / 100) +
                    Math.floor(y / 400) +
                    TimePoint.DayTotal(nYear)[nMonth - 1] +
                    nDay -
                    1) *
                    TimePoint.TickPerDay +
                nHour * TimePoint.TickPerHour +
                nMinute * TimePoint.TickPerMinute +
                nSecond * TimePoint.TickPerSecond +
                nMillisecond * TimePoint.TickPerMillisecond;
        }
        return this;
    }
    ...
    AddTick(n: number) {
        ...
        if (nNew >= TimePoint.TickMin && nNew <= TimePoint.TickMax)
            this.Tick = nNew;
        return this;
    }
    ...
    get FileTime() {
        if (this.Tick >= TimePoint.FileTimeOffset)
            return this.Tick - TimePoint.FileTimeOffset;
        else return 0;
    }
