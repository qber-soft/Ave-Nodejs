...
export function devCompileResource(sourceDir: string, outDir: string) {
    ...
    const args = [
        ...
        "-idr 4",
        `-e *.svg`,
        `-ts ResId "${path.resolve(outDir, "./ResId.ts")}"`,
        `-t "${path.resolve(outDir, "./AppRes.txt")}"`,
        `-b "${path.resolve(outDir, "./AppRes.index")}"`,
        `-l "${path.resolve(outDir, "./AppRes.log")}"`,
        `"${sourceDir}"`,
    ].join(" ");
...
export function prodCompileResource(sourceDir: string, outDir: string) {
    ...
    const args = [
        `-o "${sourceDir}"`,
        `"${path.resolve(outDir, "./AppRes.index")}"`,
        `"${path.resolve(outDir, "./AppRes.bin")}"`,
    ].join(" ");
