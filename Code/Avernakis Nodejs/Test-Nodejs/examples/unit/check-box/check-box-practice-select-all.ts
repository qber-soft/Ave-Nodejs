...
export function main(window: Window) {
	...
	const updateCheckBoxes = (checkAll: boolean) => {
		checkBoxes.forEach((each) => each.SetValue(checkAll ? CheckValue.Checked : CheckValue.Unchecked));
	};
	...
	const checkBoxCallback = (sender: CheckBox) => {
		...
		updateCurrentChecked(sender.GetText(), checkValue === CheckValue.Checked);
	};
