...
class PageRichListBox extends PageHelper<PageRichListBox> {
    ...
    OnCreateControl(window: Window) {
        ...
        const cboView = grid
            .ControlAdd(
                new ComboBox(window, new StringKey("PageRichListBoxView", 0, 5))
            )
            .SetGrid(0, 0)
            .GetControl();
        cboView.Append("", "", "", "", "");
        ...
        const icons = [
            window.CacheIcon(new IconSource(ResId.Icon_Clock_png, 16)),
            window.CacheIcon(new IconSource(ResId.Icon_Clock_png, 32)),
            window.CacheIcon(new IconSource(ResId.Icon_Clock_png, 48)),
        ];
        let nIconIndex = 0;

        ctl = grid
            .ControlAdd(new RichListBox(window))
            .SetGrid(0, 2)
            .GetControl();
        ctl.GetHeader().Add(
            new HeaderItem(
                HeaderItemFormat.Right,
                "ID",
                DpiSize.FromPixelScaled(80)
            )
        );
        ctl.GetHeader().Add(
            new HeaderItem(
        ...
        ctl.SetIconSize(
            RichListBoxViewMode.Detail,
            DpiSize.FromPixelScaled(16)
        );
        ...
        cboView.OnSelectionChange((s) => {
            ...
            switch (n) {
                ...
                case 2:
                    nIconIndex = 0;
                    ctl.SetViewMode(RichListBoxViewMode.Icon).SetIconSize(
                        RichListBoxViewMode.Icon,
                        DpiSize.FromPixelScaled(16)
                    );
                    break;
                case 3:
