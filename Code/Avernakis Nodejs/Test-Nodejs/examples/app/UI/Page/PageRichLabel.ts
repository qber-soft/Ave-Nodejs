...
class PageRichLabel extends PageHelper<PageRichLabel> {
    ...
    OnCreateControl(window: Window) {
        ...
        ctl.IconSetManager((sender, name) => {
            switch (name) {
                case "a": return ResId.Icon_Copy_png;
                case "b": return ResId.Icon_Cut_png;
                case "c": return ResId.Icon_Language_png;
                default: return 0;
            }
        });

        ctl.VaSetManager((sender, qv) => {
            if (!qv.IsName && 1 == qv.Id)
                return "<ic %c><c f00>c</c></ic> Open";
            return "";
        });
        ...
        ctl.SetText( ""
            ...
            + "<b>Bold text</b>, <i>Italic text</i>.</fx %w> <u>Underline</u>, <s>Strikethrough</s>, "
